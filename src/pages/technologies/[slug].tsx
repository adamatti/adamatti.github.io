...
function JobsSection({ slug, jobs }: { slug: string; jobs: Job[] }): ReactElement {
  return (
    <>
      {jobs && jobs.length > 0 && (
...
function EventsSection({ slug, events }: { slug: string; events: EventRecord[] }): ReactElement {
  return (
    <>
...
function TechSection({ slug, technologies }: { slug: string; technologies: Technology[] }): ReactElement {
  return (
    <>
      {technologies && technologies.length > 0 && (
        <Section title={`Technologies related to ${slug}`}>
          ...
          <p className="mt-5">
            Check all technologies I worked with on <Link href="/technologies">/technologies</Link>
          </p>
        </Section>
      )}
    </>
...
function PostsSection({ slug, posts }: { slug: string; posts: BlogPost[] }): ReactElement {
  return (
    <>
...
function VideoSection({ slug, videos }: { slug: string; videos: Video[] }): ReactElement {
  return (
    <>
...
function PetProjectsSection({ slug, projects }: { slug: string; projects: PetProject[] }): ReactElement {
  return (
    <>
      {projects && projects.length > 0 && (
        <Section title={`My pet projects related to ${slug}`}>
          ...
          <p className="mt-5">
            Check all the other projects on <Link href="/pet-projects">/pet-projects</Link>
          </p>
        </Section>
      )}
    </>
