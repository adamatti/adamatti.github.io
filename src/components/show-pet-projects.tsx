...
function ShowTags({ project: p }: { project: PetProject }): ReactElement {
  ...
  return (
    <div className="grid grid-cols-5 place-content-center gap-2">
      {tags.map((t) => (
        <div key={t} className="bordered rounded bg-cyan-400 text-white w-20 text-center text-xs">
          {t}
        </div>
      ))}
    </div>
...
export default function ShowPetProjects({ projects }: { projects: PetProject[] }): ReactElement {
  return (
    <div>
      {projects.map((p) => (
