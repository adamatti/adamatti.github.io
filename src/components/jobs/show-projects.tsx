...
export default function ShowProjects({ job, filter }: { job: Job; filter?: string }): ReactElement {
  ...
  if (filter) {
    projects = projects.filter((p) => JSON.stringify(p).toLowerCase().includes(filter));
  }
