...
function TechCard({ tech: t, addLink = false }: { tech: Technology; addLink: boolean }): ReactElement {
  const color = `shadow-${t.color ?? 'black-500'}`;
  const nameWithLink = (
    <Link href={`/technologies/${t.name.toLocaleLowerCase()}`} className="text-black">
      {t.name}
    </Link>
  );
